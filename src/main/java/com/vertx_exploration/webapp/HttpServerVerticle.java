...
public class HttpServerVerticle extends AbstractVerticle {
	...
	/*private Future<Void> prepareDatabase() {
		...
		dbClient = JDBCClient.createShared(this.vertx, new JsonObject().put("url", "jdbc:hsqldb:file:db/wiki")
	 ...
	 * router.post("/delete").handler(this::pageDeletionHandler);
	 * 
	 * server.requestHandler(router::accept).listen(8080, ar -> { if
	 * (ar.succeeded()) { LOGGER.info("HTTP server running on port 8080");
	 * future.complete(); } else { LOGGER.error("Could not start a HTTP server",
	 * ar.cause()); future.fail(ar.cause()); } });
	 * 
	 * return future; }
	 */
